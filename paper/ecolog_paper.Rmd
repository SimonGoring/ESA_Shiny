...
dtm.pred <- data.frame(dtm.s99[all_runs$msg,]>0)
...
# predict_tt <- predict(is.job.rf, dtm.pred) # this has a high error rate!!
# predict_in <- predict(is.int.rf, dtm.pred) # better than above, but still bad
# pred_good <- (as.numeric(predict_tt) + as.numeric(predict_in))^2
