...
tots = aggregate(value ~ date + variable, dat, sum)

int = dat[dat$int==TRUE,c('variable', 'date', 'value')]
dat_org = merge(int, tots, by = c('date', 'variable'))
colnames(dat_org)[3:4] = c('int', 'total')
dat_org = data.frame(dat_org, nint = dat_org$total - dat_org$int)

# totals
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=total)) + facet_grid(variable~.)
# int only
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=int)) + facet_grid(variable~.)
# nint only
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=total-int)) + facet_grid(variable~.)
# prop nint to total
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=(total-int)/total)) + facet_grid(variable~.)
# prop int to total
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=(int)/total)) + facet_grid(variable~.)

# prop nint to total
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=(total-int)/total, colour=variable)) #+ facet_grid(variable~.)
# prop int to total
ggplot(dat_org) + geom_line(data=dat_org, aes(x=date, y=int/total, colour=variable)) #+ facet_grid(variable~.)

levels(dat_org$variable) <- c('GR', 'PD', 'TT')

#####################################################################################################################
## GR
#####################################################################################################################
gr_dat = dat_org[which(dat_org$variable == 'GR'),]
...
## PD
#####################################################################################################################
pd_dat = dat_org[which(dat_org$variable == 'PD'),]
...
## TT
#####################################################################################################################
tt_dat = dat_org[which(dat_org$variable == 'TT'),]
