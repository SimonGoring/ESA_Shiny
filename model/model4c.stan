...
//   beta_tt  ~ normal(0, 5);
  ...
  // }
    ...
    mu_tt[t] ~ normal(alpha_all + alpha_ind[1] + (time_slope_ind[1]) * t/T, sigma[1]);
    mu_pd[t] ~ normal(alpha_all + alpha_ind[2] + (time_slope_ind[2]) * t/T, sigma[2]);
    mu_gr[t] ~ normal(alpha_all + alpha_ind[3] + (time_slope_ind[3]) * t/T, sigma[3]);
  }
