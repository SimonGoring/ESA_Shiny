...
//     
    mu_tt[t] ~ normal(alpha_all + alpha[1] + year_slope[1] * year[t] + year_effect[year[t]] + year_ind[1, year[t]] + X[t] * beta[1] + X[t] * beta_month , sigma[1]);
    mu_pd[t] ~ normal(alpha_all + alpha[2] + year_slope[2] * year[t] + year_effect[year[t]] + year_ind[2, year[t]] + X[t] * beta[2] + X[t] * beta_month, sigma[2]);
    mu_gr[t] ~ normal(alpha_all + alpha[3] + year_slope[3] * year[t] + year_effect[year[t]] + year_ind[3, year[t]] + X[t] * beta[3] + X[t] * beta_month, sigma[3]);
  }
