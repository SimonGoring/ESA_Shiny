...
We classified `r nrow(all_runs)` records by hand using a Shiny [@chang2015shiny] application.  Messages were identified as academic job ads, non-academic job ads (generally), and non-job postings. Academic job adds were classified into undergraduate, graduate, postdoctoral, and tenure track positions, and then also classified as interdisciplinary or not.  In addition, we provided the opportunity to classify salary type, but do not present this data here.  
...
for (i in 2:4) {
  for (j in 1:i) {
    # Why am I picking the first 87 rows?  Because that's the top 2%
    # of terms:
    cross <- sum(imp_table[1:87,i] %in% imp_table[1:87,j])
    cross_over[i,j] <- round(cross / 87 * 100, 1)
  }
}
...
imp_table <- data.frame(Tenure   = tail(names(sort(importance(is.job.rf)[,])), 
                                        n = 20),
                        PostDoc  = tail(names(sort(importance(is.pdc.rf)[,])), 
                                        n = 20),
                        Graduate = tail(names(sort(importance(is.gra.rf)[,])), 
                                        n = 20),
                        Interdisciplinary = tail(names(sort(importance(is.int.rf)[,])),
                                                 n = 20),
...
##  Note - add the most common ending to the stemmed words.
...
total.job <- rowSums(all_cast[,-c(1, 3, 4)])
...
all_melt <- subset(all_melt, date <= 2015 & !variable %in% c('Other job', "Non-job", "SE_unp"))
all_melt$int <- regexpr("Int", all_melt$variable) > 0

all_melt$variable <- substr(all_melt$variable, 1, 2) # Just get the first two letters.
...
all_melt$total_class <- NA

for (i in 1:nrow(all_melt)) {
  good_match <- which(all_melt[,1] == all_melt[i, 1] &
                        all_melt[,2] %in% all_melt[i,2])
  
  all_melt$total_class[i] <- sum(all_melt$value[good_match])

}

source('R/detrending_model.R')

all_melt$class <- factor(all_melt$class, labels = c("Graduate", 'Postdoctoral', 'Tenure Track'))

saveRDS(all_melt, 'data/all_melt.RDS')

ggplot(all_melt, aes(x = date, y = I(value/total_class))) + 
  ...
  coord_cartesian(xlim = c(2000, 2015), ylim = c(0, 1), expand = 0) +
  xlab('Year') +
...
**Figure 2.** *Job postings as a proportion of postings within each class on ECOLOG.*

```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(all_melt, aes(x = date, y = I(value/(total.job)))) + 
  geom_point(aes(color = int), alpha = 0.5) +
  geom_smooth(span = 0.16, 
              aes(linetype = int),
              se = FALSE, size = 1) +
  theme_bw() + facet_wrap(~class) +
  scale_linetype_discrete(guide = guide_legend(title = "Interdisciplinary")) +
  ...
  scale_x_continuous(breaks = seq(2000, 2010, by = 5)) +
  scale_y_continuous(breaks = seq(0.05, .25, by = .05)) +
  coord_cartesian(xlim = c(2000, 2015), ylim = c(0, 0.5), expand = 0) +
  xlab('Year') +
  ylab('Proportion of Job Ads') +
  theme(axis.text = element_text(family = 'serif', size = 16),
        axis.title = element_text(family = 'serif', size = 18, face = 'bold'),
        #legend.text = element_text(family='serif', size = 14),
        strip.text =  element_text(family = 'serif', size = 16, face = 'bold'))
...
**Figure 3.** *Job postings as a proportion of all postings on ECOLOG, including non-job postings.*

Seasonal trends suggest differences in timing and intensity for the different job classes (Figure 4).  In all cases the amplitude of the disciplinary curve is much higher than the amplitude of the interdisciplinary curve, although this may be, in part, a function of the total number of postings.  The timing of peak posting and the lowest rates of posting also varies by class.  The lowest proportion of postings for graduate positions occurs at the end of July (as with Postdoctoral positions), while the lowest volume for Tenure Track positions occurs a month earlier in June.  The peak time of tenure track posting occurs in mid-November (similar to Postdoctoral postings), while the graduate posting peak appears to be broader, with a peak in late February.

```{r, echo=FALSE, message=FALSE, warning=FALSE}

ggplot(all_melt, aes(x = month, y = I(value/(total.job)))) + 
  geom_point(aes(color = int), alpha = 0.5) +
  geom_smooth(span = 0.16, 
              aes(linetype = int),
              se = FALSE, size = 1) +
  ...
  #scale_x_continuous(breaks = seq(2000, 2010, by = 5)) +
  scale_y_continuous(breaks = seq(0.05, .25, by = .05)) +
  coord_cartesian(xlim = c(1, 12), ylim = c(0, 0.5), expand = 0) +
  xlab('Month') +
  ylab('Proportion of Job Ads') +
  theme(axis.text = element_text(family = 'serif', size = 16),
        axis.title = element_text(family = 'serif', size = 18, face = 'bold'),
...
**Figure 5** *Annual trend of interdisciplinary postings as a proportion within job classes.*
...
In STEM fields generally, there has been a recognition of an academic career bottleneck, where there are many graduate training positions, but not enough jobs for these students to move into after graduation (BLS 2015). In recent years, increased duration of faculty careers (Syst Res Behav Sci 2014) and limited access to funding, means that the glut of PhDs have been absorbed into postdoctoral positions (BLS 2015), lengthening the amount of time that an early career researcher stays in a postdoctoral position across STEM disciplines (Kozorovitskiy 2015, Syst Res Behav Sci 2014, Laudel and Glaser 2008). By some estimates, ther are currently 60,000 - 100,000 postdoctoral researchers in the United States alone (Science 2015), although an exact measure is unavailable since postdoctoral researchers themselves are difficult to define (e.g. academic, govt, temporary jobs, contract jobs, soft money - I'm sure there's a ref for this somewehre). Researchers who spend longer in postdoctoral positions, often moving several times during this career phase, cite job insecurity, lack of career structure, and inadequate mentoring as major concerns to staying active in STEM research (Scaffidi and Berman 2009, Ackerlind 2009, Kozorovitskiy 2015). In addition, overall dissatisfaction with the academic job market may have led to an estimated 5.5% loss in the (united states?) STEM postdoctoral population from 2010-2013 (Science 2015).

Despite an increased emphasis on interdisciplinary training at the graduate and postdoctoral levels (refs), employers in STEM fields still often prefer to hire scientists who are established within a specific disciplinary silo (Nelson 2011). Cluster hiring is a growing trend in academic faculty searches (we did not record data on whether a TT ad was part of a cluster hire), often with the express goal of hiring diverse, interdisciplinary researchers (Flaherty 2015; Urban Universities for Health 2015). However, cluster hires are still a relatively new hiring practice (?) and interdisciplinary hiring decisions were found to frequently be influenced by doubts of department faculty and administrators, or concerns about the ability of the department to provide adequate funds for start-up and additional hires (Flaherty 2015). Conversely, outside of academia, there is perhaps an increasing demand for researchers who have interdisciplinary training (Nugent & Kulkarni 2013). This doesn't mean that interdisciplinary trainees necessarily are less likely to ultimately be hired into academic positions, but it does mean that often, special care must be taken to ensure that the job candidate's CV reflects deep knowledge/establishment within a discipline (). Even once a tenure-track hire is made, an interdisciplinary researcher may need to manage their promotion process differently than a researcher who is disciplinarily focused (e.g. more depands on time, service to multiple departments, etc.; Pollack & Snir YR?, Pfirman and Begg 2012). 
...
**One paragraph** (*could this be a Box?*)

For academic tenure-track jobs, interdisciplinary early career researchers are often advised to present their training in a way that allows hiring committees to see them as fitting naturally into a disciplinary home (Kelsky 2014, Cawley 2015). This may be especially important if tenure-track hiring typically follows the siloed model (where disciplinary candidates are preferred) or the balanced model (where there are fewer interdisciplinary than disciplinary hires, so interdisciplinary candidates are likely applying for both types of jobs). During the interview, an interdisciplinary candidate may choose to highlight their ability to work between departments and fields, but the most important consideration for hiring still likely lies in having a focused disciplinary research area, and to propose interesting and novel work within the department (Kelsky 2014, Nelson 2011, Cawley 2015). Interdisciplinary researchers may benefit from seeking additional mentoring from someone outside their main discipline, and/or from an established interdisciplinary academic researcher, to discuss how to best leverage their disciplinary vs. interdisciplinar training on the job market (Pfirman and Begg 2012). In addition, some departments include specific references to "fostering interdisciplinarity" in their mission statements, which might indicate stronger faculty and administrative support for hiring an interdisciplinary candidate (Pfirman and Begg 2012). Finally, if hired, interdisciplinary researchers should be aware that most departmental incentive structures for tenure and promotion are still largely disciplinary focused, even when they have hired an interdisciplinary candidate (Pfirman and Begg 2012).
...
##Acknowledgements
SJG developed the idea behind this paper, contributed to the hand classification of postings, the bulk of the code and to writing and editing of this paper. SRS helped with the framing, hand classification and writing of the paper. AD assisted with the framing, writing and statistical analysis (to do). DI contributed to the framing of the paper, assisted in writing and editing the draft paper.
