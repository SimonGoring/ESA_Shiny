...
Using the surveyed messages (n=XXXX), we built a model using a random forest to classify messages as tenure track jobs, postdoctoral positions and graduate positions, and to test whether messages indicate interdisciplinarity (package *randomForest*; Liaw and Wiener 2002 [@LiawWiener2002RF]).  Based on the model constructed we then predict the class of the remaining messages to build a time series of job ads.  Since hand coding messages was an iterative process, we used the confusion matrix from the predictive models to weight the messages selected for display and classification.  Messages that were predicted to be interdisciplinary from the partial models were weighted most highly, as were messages with high rates of confusion for tenure track, postdoctoral or graduate positions.
...
* declining number of opportunities at each academic career stage (more grad positions than postdoc than t-t).
...
* are interdiscp. researchers seeking non t-t careers (e.g. government, non profit?)
...
* refs on ac hiring trends generally (e.g. proportion of ecology grads/postdocs that get t-t jobs)
